...
class MetarSensor(SensorEntity):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                with async_timeout.timeout(10):
                    ...
                    # Clean the response to extract only the METAR string
                    metar_string = self._extract_metar(raw_data)

                    # Parse the METAR data using python-metar
                    report = Metar(metar_string)

                    # Safely extract attributes from the METAR object
                    self._attributes = {
                        ...
                        "sky": ", ".join(str(layer) for layer in report.sky),
                        "remarks": ", ".join(report.remarks()),
                        "raw_metar": report.code,
                    }

                    # Set the state to temperature or "N/A"
    ...
    def _extract_metar(self, raw_data):
        ...
        # Assume METAR is the last line in the response.
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async_add_entities([MetarSensor(station_code)])
